...
import static luki.x.XConfig.HOLDER_PARSER_KEY;
...
public class InjectUtils {
	...
	public static Object getValueByTag(String field, Object data) {
		...
		Object value = null;
		if (KEY_THIS.equals(field)) return data;
		...
		try {
			for (int i = 0; i < fields.length; i++) {
				...
				try {
					if (!KEY_THIS.equals(field)) data = ReflectUtils.getFieldValue(data, field);
				} catch (Exception e1) {
					...
					for (int j = 1; split != null && j < split.length; j++) {
						if (data instanceof List) {
							...
							if (list.size() > toInt(split[j])) {
								...
								XLog.v(TAG, "List's size('" + list.size() + "') < index('" + split[j] + "') in '" + field
							...
							if (objs.length > toInt(split[j])) {
								...
								XLog.v(TAG, "Object array's length('" + objs.length + "') < index('" + split[j] + "') in '" + field
										+ "'. Please check carefully !!  ");
							}
			...
			if (e.getCause() == null) {
				XLog.w(TAG,
						"can't find this field or method '" + field + "' in " + class1 + " . Please check carefully !! \n " + e.toString());
			} else
	...
	public static ParseHolder getParserHolder(Class<? extends ParseHolder> clazz, String key, View v) {
		if (v != null && v.getTag(HOLDER_PARSER_KEY) != null && v.getTag(HOLDER_PARSER_KEY) instanceof ParseHolder) {
			return (ParseHolder) v.getTag(HOLDER_PARSER_KEY);
		}
		ParseHolder holder = createParserHolder(clazz, null);
		holder.onParse(key);
		if (v != null) {
			v.setTag(HOLDER_PARSER_KEY, holder);
		}
