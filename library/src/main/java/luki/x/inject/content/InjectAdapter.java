...
import static luki.x.XConfig.SCREEN_WIDTH;
...
public class InjectAdapter<T> extends BaseAdapter implements IXAdapter<T> {
	...
	/**
	 * {@link #getColumnCount()} 大于 1 的时候 返回的行高
	 *
	...
	private View[] createViews(int position, int rowCount, View[] views, ViewGroup parent) {
		...
		for (int i = 0; i < rowCount; i++, position++) {
			if (position < mData.size()) {
				...
				views[i].setTag(HOLDER_POSITION, position);
				...
				views[i].setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						int position = (Integer) v.getTag(HOLDER_POSITION);
	...
	protected View getItemView(final int position, View convertView, ViewGroup parent) {
		...
		convertView.setTag(HOLDER_POSITION, position);
