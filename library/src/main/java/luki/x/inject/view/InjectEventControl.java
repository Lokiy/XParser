...
import luki.x.inject.content.annotation.XCDParser;
import luki.x.inject.content.annotation.XCDString;
import luki.x.inject.view.annotation.ListenerInject;
import luki.x.inject.view.annotation.ListenerInject.ListenerType;
import luki.x.inject.view.annotation.Select;
import luki.x.inject.view.annotation.ViewInject;
import luki.x.util.ReflectUtils;
import luki.x.util.WidgetUtils;
import luki.x.util.WidgetUtils.ResType;
			...
			for (Field field : fields) {
				...
				if (viewInject != null) {
					...
					if (!TextUtils.isEmpty(select.selected())) {
						...
						setViewListener(activity, ListenerType.NOSELECT, field, select.noSelected());
					}
				}
			}
		}
